...
export const toObjectId = (id: string | ObjectId | mongoose.Types.ObjectId | Uint8Array) => mongoose.Types.ObjectId.createFromHexString(id.toString());
...
// export const createName = (firstName: string, lastName: string) => [firstName, lastName].filter((i) => !!i.trim()).join(' ');
export const createName = (firstName: string, lastName: string) => [firstName || '', lastName || ''].join(' ').trim();
