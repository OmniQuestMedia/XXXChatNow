...
implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleDisconnect(client: Socket) {
    ...
    await this.logout(client, client.handshake.query.token as string);
  }

  async login(client: Socket, token: string) {
    const authUser = this.authService.decodeJWT(token);
    if (!authUser) {
      return;
    }
  ...
  async logout(client: Socket, token: string) {
    ...
    const connections = await this.socketUserService.getUserSocketIds(
      authUser.sourceId.toString()
    );
    ...
    // remove disconected sockets
    await connections.reduce(async (lp, connection) => {
      await lp;
      if (!this.socketUserService.hasConnected(connection)) {
        await this.socketUserService.removeConnection(
          authUser.sourceId,
          connection
        );
      }
