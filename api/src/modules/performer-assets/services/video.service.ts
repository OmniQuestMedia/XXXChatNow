...
export class VideoService {
  ...
  public async findByIds(ids: string[] | ObjectId[]): Promise<VideoDto[]> {
    const videos = await this.VideoModel
      ...
      .lean()
      .exec();
    return videos.map((p) => plainToInstance(VideoDto, p));
  }
  ...
  public async findById(id: string | ObjectId): Promise<VideoDto> {
    const video = await this.VideoModel
      ...
      .lean()
      .exec();
    return plainToInstance(VideoDto, video);
  }
  ...
  public async handleFileProcessed(event: QueueEvent) {
    try {
      ...
      this.logger.error(e.stack || e, { context: 'VideoService' });
    }
  }
  ...
  private getVideoForView(fileDto: FileDto, videoId: string | ObjectId, jwToken: string) {
    ...
    return {
      ...
      mineType: fileDto.mimeType,
      absolutePath: fileDto.absolutePath,
  ...
  public async create(
    ...
    const watermark = await this.watermarkService.getWatermarkOptions(model.performerId);
  ...
  public async userGetDetails(videoId: string | ObjectId, currentUser: UserDto, jwToken: string): Promise<VideoDto> {
    ...
    const isBought = !dto.isSaleVideo ? true : await this.paymentTokenService.checkBoughtVideo(dto._id, currentUser);
    dto.setInfo({
      jwt: jwToken,
      includeFileUrl: true,
      thumbnail: thumbnailFile,
  ...
  public async countTotalVideos(query: FilterQuery<Video> = {}): Promise<number> {
