...
export class CreateOrderFromPurchasedItemListener {
  ...
  public async handler(event: QueueEvent) {
    try {
      ...
      const order = new this.OrderModel({
        ...
        postalCode: purchasedItem.extraInfo?.postalCode,
      ...
      await this._emailNotification(dto, product);
    } catch (error) {
  ...
  private async _emailNotification(order: OrderDto, product: ProductDto) {
    ...
    if (user) {
      ...
      await this.mailService.send({
        ...
        to: user.email,
        data: {
          user: new UserDto(user).toEmailResponse(),
          order,
          digitalLink
        },
        template
