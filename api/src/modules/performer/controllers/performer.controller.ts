...
export class PerformerController {
  ...
  // @Roles('performer')
  // @UseGuards(RoleGuard)
  async me(@Request() request: Req): Promise<DataResponse<Partial<PerformerDto>>> {
    ...
    result.privateCallPrice = typeof result.privateCallPrice !== 'undefined' ? result.privateCallPrice : defaultC2CPrice;
    result.groupCallPrice = typeof result.groupCallPrice !== 'undefined' ? result.groupCallPrice : defaultGroupChatPrice;

    return DataResponse.ok(new PerformerDto(result).toResponse(true));
  }

  @Get('/search/new-models')
  @HttpCode(HttpStatus.OK)
  @UseInterceptors(UserInterceptor)
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async userSearchNewModel(
    ...
    const data = await this.performerSearchService.searchNewModel(
      query,
      user,
      countryCode
    );
  ...
  async userSearch(
    ...
    const resp = await this.performerSearchService.advancedSearch(query, user, countryCode);
  ...
  async uploadPerformerDocument(
    ...
    return DataResponse.ok({
      ...file,
      url: `${file.getUrl()}?documentId=${file._id}&token=${request.headers.authorization
      }`
    });
  }
  ...
  async deleteUserBlock(
    ...
    const data = await this.performerService.removeUserBlockedList(
      performer._id,
      userId
    );
    return DataResponse.ok(data);
  ...
  @Get('/search-by-interest')
  @HttpCode(HttpStatus.OK)
  @UseInterceptors(UserInterceptor)
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async getPerformersBasedOnUserPreferences(
    @Query() req: PerformerSearchPayload,
    @CurrentUser() user: UserDto
  ): Promise<DataResponse<PageableData<Partial<PerformerDto>>>> {
    const resp = await this.performerService.getPerformersBasedOnUserPreferences(user._id, req);
    return DataResponse.ok(resp);
  }

  @Get('/:performerId/check-user-ranking')
  @UseGuards(AuthGuard)
  @HttpCode(HttpStatus.OK)
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async checkUserRank(
    ...
    const userRank = await this.performerService.checkUserRank(performerId, user._id);
    return DataResponse.ok(userRank);
  }
}
