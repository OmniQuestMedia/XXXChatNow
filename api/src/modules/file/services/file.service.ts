...
export class FileService {
  ...
  public async createFromMulter(
    ...
    if (options.replaceWithThumbail && options.generateThumbnail && options.thumbnailSize) {
      ...
      unlinkSync(multerData.path);
      writeFileSync(multerData.path, buffer as Buffer);
    }
  ...
  private async _processPhoto(event: QueueEvent) {
    ...
    try {
      ...
      if (options.watermark) {
        const watermarkImage = await this.imageService.addWatermark(photoPath, options.watermark);
        unlinkSync(photoPath);
        writeFileSync(photoPath, watermarkImage);
      }
      ...
      writeFileSync(join(photoDir, thumbName), buffer as Buffer);
      await this.FileModel.updateOne(
        { _id: fileData._id },
        {
          $set: {
