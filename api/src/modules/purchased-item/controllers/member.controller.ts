...
import { SendTipsPayload, SendContributePayload } from '../payloads';
...
export class MemberPaymentToken {
  ...
  @Post('/send-contribute-token/:id')
  @HttpCode(HttpStatus.OK)
  @Roles('user')
  @UseGuards(RoleGuard)
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async sendTokenContribute(
    @CurrentUser() user: UserDto,
    @Param('id') id: string,
    @Body() payload: SendContributePayload
  ): Promise<DataResponse<any>> {
    const data = await this.paymentService.sendTokenContribute(user, id, payload);
    return DataResponse.ok(data);
  }
  ...
  async peekIn(
    ...
    const info = await this.paymentService.peekIn(user, id);
    return DataResponse.ok(info);
  }
}
