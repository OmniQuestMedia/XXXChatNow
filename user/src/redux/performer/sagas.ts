    ...
    * worker(data: IReduxAction<IPerformer>) {
      try {
        ...
        const [
          ...
          productService.search({ performerId: data.payload._id, status: 'active' }),
          videoService.search({ performerId: data.payload._id, status: 'active' }),
          galleryService.search({ performerId: data.payload._id, status: 'active' }, false),
        ...
        performerProductResponse.data.data.forEach((p) => (products[p._id] = p));
        ...
        yield put(
          getPerformerDetailsSuccess({
            performer: {
              ...
              products: performerProductResponse.data.data.map((p) => p._id),
