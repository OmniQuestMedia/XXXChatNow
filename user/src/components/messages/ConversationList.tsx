...
class ConversationList extends PureComponent<IProps> {
  ...
  onMessage = async (message: { conversationId: string | number; text: any, senderId: string }) => {
    if (!message) {
      return;
    }
    const {
    ...
    (message.senderId === currentUser._id || message.senderId === currentPerformer._id) && await messageService.readAllInConversation(conversationId, currentUser._id ? currentUser._id : currentPerformer._id);
  };
  ...
  render() {
    ...
    if (!this.conversationsRef) this.conversationsRef = createRef();
    return (
      <div className={style['conversation-list']} ref={this.conversationsRef} onScroll={this.handleScroll.bind(this)}>
        <Event event="message_created" handler={this.onMessage} />
        <h4 className={style['conversation-title']}>Messenger</h4>
        <ConversationSearch
          onSearch={(e) => {
            e.persist();
        ...
        {!requesting && !conversations.length && <p className={style['text-center']}>No conversation found.</p>}
      </div>
    );
  }
}
