...
import style from './products-form.module.less';
...
function ProductForm({ onFinish, loading, product }: IProps) {
  let inputRef;
  let selectRef;
          ...
          dropdownRender={(menu) => (
            <>
              ...
              <Input
                ...
                onPressEnter={() => {
                  let token = parseInt(inputRef.input.value, 10);
                  if (token < 1) token = 1;
                  setTimeout(() => {
              ...
              <Form.Item>
                <div>
                  {!digitalFileMetaData.length && product?.digitalFile && (
                    <a className={style['upload-item-name']} href={product.digitalFile} download target="_blank" rel="noreferrer">
...
ProductForm.authenticate = true;
ProductForm.layout = 'primary';
