...
class StreamService extends APIRequest {
  ...
  getPublicStream(performerId: string) {
    return this.get(`/streaming/public/${encodeURIComponent(performerId)}`);
  }

  peekIn(performerId: string) {
    return this.post(`/streaming/private/peek-in/${performerId}`);
  }

  getPeekIn(id: string, header: any) {
    return this.get(`/streaming/private/peek-in/${id}`, header);
  }

  payPeekIn(id: string) {
    return this.post(`/member/peek-in/${id}`);
  }
