...
export class PerformerService extends APIRequest {
  ...
  searchNewModel(query?: { [key: string]: any }) {
    return this.get(this.buildUrl('/performers/search/new-models', query));
  }
  ...
  removeProduct(id: string) {
    return this.del(
      `/performer/performer-assets/products/${encodeURIComponent(id)}`
    );
  }
  ...
  checkUserRank(performerId: string) {
    return this.get(`/performers/${performerId}/check-user-ranking`);
  }
  ...
  loadWatermarkerConfigs() {
    return this.get('/watermark-settings');
  }

  updateWatermarkerConfigs(data, file?: any) {
    const files = [];
    if (file) {
      files.push({
        file,
    ...
    return this.upload('/watermark-settings', files, {
      customData: data,
      onProgress: () => true
    });
  }
