...
export class AdminPerformerController {
  ...
  async searchOnline(
    ...
    const conversations = await Promise.all(
      performerIds.map((id) => this.conversationService.findPerformerPublicConversation(id))
    );
    const watchings = await Promise.all(
      performerIds.map((id, index) => (conversations[index] ? this.socketUserService.countRoomUserConnections(id.toString()) : 0))
    );
