...
class PrivateStreamingContainer extends PureComponent<IProps, IState> {
    ...
    this.socket.on(
      ...
      ({ streamId, streamList, conversationId: _id }) => {
        ...
        if (streamList.length) {
          ...
          this.setState({ modelJoined: true });
        }
      }
    );
    this.socket.on(
  ...
  render() {
    const {
      onClick, initialized, containerClassName, configs,
      requesting, performer, requestFromUser
    } = this.props;
    const { loading, modelJoined, stopping } = this.state;
    return (
      <div className={classnames(style['stream-private-main'], {
        [style['stream-started']]: modelJoined
      })}
      >
        <div className="stream-load-video">
          <div
            className={classnames(
              'private-streaming-container',
              style['private-streaming-container'],
              containerClassName
            )}
          >
            <video
        ...
        <div className="button-start-stream">
          {requestFromUser && performer?.isOnline && (
            !initialized ? (
              <div className={style['private-container']}>
                <Button
                  type="primary"
                  onClick={() => {
                    if (performer.streamingStatus === 'private') {
                      message.error('Model is streaming private, please connect after some time');
                      return;
                    }
                    onClick();
                  }}
              ...
              <Button
                ...
                block
                disabled={stopping || loading || requesting}
              >
                Stop Streaming
              </Button>
            )
          ...
          {!requestFromUser && (
            !initialized ? (
              <div className={style['private-container']}>
                <Button type="primary" onClick={onClick} loading={loading || requesting} block>
