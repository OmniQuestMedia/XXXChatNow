...
export class ProductService extends APIRequest {
  details(id: string, headers?: { [key: string]: string; }) {
    return this.get(`/performer/performer-assets/products/${encodeURIComponent(id)}/view`, headers);
  }
  ...
  getDownloadLink(id: string) {
    return this.get(`/user/performer-assets/products/${encodeURIComponent(id)}/download-link`);
  }
}
